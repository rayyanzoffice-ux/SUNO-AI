...
void main() {
  test('runtime demo scenarios preserve LOW MEDIUM CRITICAL outcomes',
      () async {
  ...
  test('named demo scenarios stay frozen when a live repository is injected',
      () async {
  ...
  test("CAN'T RESPOND immediately escalates the active safety check",
      () async {
    final runtime = SunoRuntimeService(
      safetyCheckEngine: SafetyCheckEngine(
  ...
  test('I AM SAFE resolves the safety check as cancelled', () async {
    ...
    expect(
        runtime.currentIncident?.contactResponseText, 'User confirmed safe');
  });

  test(
      'updateStatus with no text preserves existing contactResponseText',
      () async {
    final runtime = SunoRuntimeService();
    final critical = await runtime.runDetection(DetectionScenario.critical);
    await runtime.recordDetection(critical);
    await runtime.updateStatus(
        IncidentStatus.contactChecking, 'Contact checking — help is on the way');
    final before = runtime.currentIncident?.contactResponseText;

    await runtime.updateStatus(IncidentStatus.contactNotified);
    expect(runtime.currentIncident?.contactResponseText, before);
  });

  test('SafetyCheckEngine disposal resolves the pending future as cancelled',
      () async {
    ...
    final engine =
        SafetyCheckEngine(countdownDuration: const Duration(seconds: 1));
  ...
  test('incident repository preserves createdAt and updates status/history',
      () async {
    ...
    expect(updated.contactResponseText,
  ...
  test('trusted contact responses persist the expected incident statuses',
      () async {
    final runtime = SunoRuntimeService();
    final critical = await runtime.runDetection(DetectionScenario.critical);
    await runtime.recordDetection(critical);

    await runtime.updateStatus(IncidentStatus.contactChecking,
        'Contact checking — help is on the way');
    expect(runtime.currentIncident?.status, IncidentStatus.contactChecking);

    await runtime.updateStatus(
        IncidentStatus.resolved, 'Resolved — contact confirmed they are safe');
    expect(runtime.currentIncident?.status, IncidentStatus.resolved);

    await runtime.updateStatus(IncidentStatus.alertTriggered,
        'Unable to contact — emergency remains active');
    expect(
        runtime.currentIncident?.status, IncidentStatus.alertTriggered);
    expect(runtime.currentIncident?.contactResponseText,
        'Unable to contact — emergency remains active');
  });
  ...
  test('runtime can add a trusted contact through its repository', () async {
    ...
    final runtime =
        SunoRuntimeService(trustedContactRepository: repository);
